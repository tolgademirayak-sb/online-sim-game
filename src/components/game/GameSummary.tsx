import { GameState, ROLE_LABELS, SUPPLY_CHAIN_ORDER } from '@/types/game';
import { calculateTotalCost, calculateSystemCost } from '@/lib/gameLogic';
import { GameCharts } from './GameCharts';
import { Button } from '@/components/ui/button';
import { Trophy, Award, TrendingDown, RotateCcw, Share2 } from 'lucide-react';
...
export function GameSummary({ gameState, onRestart }: GameSummaryProps) {
  const { stages, playerRole, totalWeeks, history } = gameState;
  ...
  const playerRank = rankings.findIndex((r) => r.role === playerRole) + 1;

  // Calculate some stats
  const playerHistory = history[playerRole];
  const maxInventory = Math.max(...playerHistory.map((h) => h.inventory));
  const maxBacklog = Math.max(...playerHistory.map((h) => h.backlog));
  const avgOrder = playerHistory.length > 0
    ? playerHistory.reduce((sum, h) => sum + h.orderPlaced, 0) / playerHistory.length
    : 0;
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8">
      <div className="max-w-6xl mx-auto space-y-8 animate-fade-in">
          ...
          <div className="glass-card rounded-xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-muted/50 rounded-lg p-4 text-center">
                ...
                <p className={cn(
                  'text-2xl font-bold',
                  playerRank === 1 ? 'text-primary' : 'text-foreground'
                )}>
                  #{playerRank}
                </p>
              </div>
              <div className="bg-muted/50 rounded-lg p-4 text-center">
                <p className="text-xs text-muted-foreground mb-1">System Cost</p>
                <p className="text-2xl font-bold text-foreground">${systemCost.toFixed(2)}</p>
              </div>
            </div>
        ...
        {/* Charts */}
        <GameCharts history={history} playerRole={playerRole} showAllRoles={true} />
